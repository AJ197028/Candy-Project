    ...
    const ModifiedCart = CartCntx.updateDetails.map((item) => {
      if (item.candyName===ItemList.candyName) {
       item.quantity+=1;
        ...
        const Email = true;
        const userEmail = Email ? "aksjais20gmailcom" : "aks";

        fetch(
          `https://crudcrud.com/api/ba25ef8330da4feb9fd6ef3d6f1a29e4/${userEmail}/${item._id}`,
          {
            method: "PUT",
            headers: {
      ...
      if (!CurrentItem) {
        ...
        const userEmail = Email ? "aksjais20gmailcom" : "aks";
  fetch(
    `https://crudcrud.com/api/ba25ef8330da4feb9fd6ef3d6f1a29e4/${userEmail}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
